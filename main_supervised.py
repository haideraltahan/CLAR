...
class Main(LightningModule):
    ...
    def load_datasets(self):
        if 'esc' in self.dataset:
            ...
            data_path = os.path.join(self.data_dir, "test_" + self.dataset + ".lmdb")
            self.val_dataset = NormalLMDBP(data_path, transform=self.test_transform, aug_transform=self.aug_transform,
                                           spec_transform=self.spec_transform)
            data_path = os.path.join(self.data_dir, "test_" + self.dataset + ".lmdb")
            self.test_dataset = NormalLMDBP(data_path, transform=self.test_transform, aug_transform=self.aug_transform,
                                            spec_transform=self.spec_transform)
